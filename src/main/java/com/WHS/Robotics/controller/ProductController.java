...
public class ProductController {
    ...
    public ResponseEntity<Resource> serveProductImage(@RequestParam String filename) {
        try {
            ...
            Path imagePath = Paths.get(realPath).resolve(decoded).normalize();
            Resource resource = new UrlResource(imagePath.toUri());
    
            if (!resource.exists() || !resource.isReadable()) {
                return ResponseEntity.notFound().build();
            }
            String contentDisposition = "inline; filename=\"" + org.springframework.util.StringUtils.cleanPath(decoded) + "\"";
